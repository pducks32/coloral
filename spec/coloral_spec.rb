...
module Coloral
  describe Coloral do
    ...
    describe "::from_hex" do
      context "with no parameters" do
        it "raises ArgumentError" do
          expect{ Coloral.from_hex() }.to raise_error ArgumentError
        end
      end
    ...
    describe "::from_hsl" do
      context "with no parameters" do
        it "raises ArgumentError" do
          expect{ Coloral.from_hsl() }.to raise_error ArgumentError
        end
      end
      context "with no special syntax" do
        it "returns a color" do
          expect(Coloral.from_hsl(175, 50, 85)).to be_a Color
        end
      end
      context "with hsl hash" do
        it "returns a color" do
          hash = { h: 175, s: 50, l: 85 }
          expect(Coloral.from_hsl(hash)).to be_a Color
        end
      end
    ...
    describe "::from_rgb" do
      ...
      context "with no special syntax" do
        it "returns a color" do
          expect(Coloral.from_rgb(200, 180, 90)).to be_a Color
        end
      end
      context "with hsl hash" do
        it "returns a color" do
          hash = { r: 200, g: 180, b: 90 }
          expect(Coloral.from_rgb(hash)).to be_a Color
        end
      end
      context "with hue saturation lightness hash" do
        it "returns a color" do
